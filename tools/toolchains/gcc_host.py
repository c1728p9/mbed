...
class GCC_HOST(mbedToolchain):
    ...
    def __init__(self, target,  notify=None, macros=None,
        ...
        tool_path=TOOLCHAIN_PATHS['GCC_HOST']
        main_cc = join(tool_path, "gcc")
        main_cppc = join(tool_path, "g++")
        ...
        self.ld = [join(tool_path, "gcc")] + self.flags['ld']
        self.sys_libs = ["stdc++", "supc++", "m", "gcc"]
        self.preproc = [join(tool_path, "cpp"), "-E", "-P"]

        self.ar = join(tool_path, "ar")
        self.elf2bin = join(tool_path, "objcopy")
    ...
    def check_executable():
        """Returns True if the executable (gcc) location
        specified by the user exists OR the executable can be found on the PATH.
        Returns False otherwise."""
        if not TOOLCHAIN_PATHS['GCC_HOST'] or not exists(TOOLCHAIN_PATHS['GCC_HOST']):
            if find_executable('gcc'):
                TOOLCHAIN_PATHS['GCC_HOST'] = ''
                return True
            else:
                return False
        else:
            exec_name = join(TOOLCHAIN_PATHS['GCC_HOST'], 'gcc')
