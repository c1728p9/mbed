...
void USBCDC::callback_set_configuration(uint8_t configuration) {
    ...
    if (configuration == DEFAULT_CONFIGURATION) {
        ...
        read_start(bulk_out, _rx_buf, sizeof(_rx_buffer));
...
void USBCDC::send_next()
{
    ...
    if (USBDevice::write_start(bulk_in, _tx_cur->tx_buf, size_to_send)) {
...
void USBCDC::send_isr(usb_ep_t endpoint)
{
    ...
    MBED_ASSERT(_tx_cur != NULL);
    uint32_t size_sent = write_finish(endpoint);
    _tx_cur->tx_size -= size_sent;
    _tx_cur->tx_buf += size_sent;
...
void USBCDC::receive_next()
{
    ...
    if ((_rx_size == 0) && !_rx_in_progress) {
        // Refill the buffer
        read_start(bulk_out, _rx_buffer, sizeof(_rx_buffer));
        _rx_in_progress = true;
    }
}
