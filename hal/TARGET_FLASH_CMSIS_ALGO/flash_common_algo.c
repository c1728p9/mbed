...
uint32_t flash_get_sector_size(const flash_t *obj, uint32_t address)
{
    ...
    const uint32_t flash_base = obj->target_config->flash_start;
    if (address < flash_base) {
        return MBED_FLASH_INVALID_SIZE;
    }

    int sector_index = obj->target_config->sector_count - 1;
    const sector_info_t *sectors = obj->target_config->sectors;
    for (; sector_index >= 0; sector_index--) {
        if (address >= sectors[sector_index].start + flash_base) {
