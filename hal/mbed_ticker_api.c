...
static const ticker_info_t default_ticker_info = {
    1000000,
    32
};
...
static void initialize(const ticker_data_t *ticker)
{
    ...
    const ticker_info_t *info = &default_ticker_info;
    if (ticker->interface->get_info != NULL) {
        info = ticker->interface->get_info();
    }
    ...
    if ((info->bits > 32) || (0 == info->bits)) {
        MBED_ASSERT(info->bits != 0);
        bits = 32;
    }
 ...
 */
static int match_interval_passed(timestamp_t prev_tick, timestamp_t cur_tick, timestamp_t match_tick)
{
...
static void schedule_interrupt(const ticker_data_t *const ticker)
{
    ...
    if (ticker->queue->head) {
        ...
        if (match_interval_passed(queue->tick_last_read, cur_tick, match_tick)) {
            ticker->interface->fire_interrupt();
        }
    } else {
        uint32_t match_tick =
