...
osThreadId_t osThreadNew (osThreadFunc_t func, void *argument, const osThreadAttr_t *attr)
{
    ...
    if ((attr->cb_mem != 0) && (attr->cb_size == sizeof(os_thread_t))) {
        thread = (os_thread_t *)attr->cb_mem;
        thread->alloc = 0;
    } else {
        thread = malloc(sizeof(os_thread_t));
        thread->alloc = 1;
    }
...
//    check_return(ret);
    ...
    if ((attr->cb_mem != 0) && (attr->cb_size == sizeof(os_mutex_t))) {
        mutex = attr->cb_mem;
        mutex->alloc = 0;
